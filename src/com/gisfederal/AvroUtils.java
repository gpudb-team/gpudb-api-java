...
public class AvroUtils {
	...
	public static Object convert_to_object(Schema type_schema, byte[] ba) throws GaiaException{		
		...
		if(type_schema == gaia_response.SCHEMA$) {			
			...
			throw new GaiaException("Unsupported type schema:"+type_schema);
		}
	...
	// the initial byte[] is assumed to be a gaia_response object
	public static Object convert_to_object_from_gaia_response(Schema type_schema, byte[] ba) throws GaiaException {		
		// decode the gaia response first
		...
		if(g_response.getStatus().toString().equals("ERROR")){
			...
			throw new GaiaException(g_response.getMessage().toString());
		}
