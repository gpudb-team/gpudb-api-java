...
public class FilterByNAIRequest extends Request {
	...
	public FilterByNAIRequest(Gaia gaia, String file, SetId in_set, SetId result_set_id, String x_attribute, List<Double> x_vector, String y_attribute, List<Double> y_vector) {
		this.gaia = gaia;
		this.file = file;		
		this.log = Logger.getLogger(FilterByNAIRequest.class);
		
		this.log.debug("gaia:"+gaia.toString()+" in_set:"+in_set.get_id());
		filter_by_nai_request request = new filter_by_nai_request(result_set_id.get_id(), in_set.get_id(), x_attribute, x_vector, y_attribute, y_vector, this.gaia.getUserAuth()); 
