...
import avro.java.gpudb.bulk_update_request;
...
public class BulkUpdateRequest extends Request {

	public BulkUpdateRequest(GPUdb gPUdb, String file, NamedSet ns, String global_expression, List<java.lang.CharSequence> expressions,
			List<Map<java.lang.CharSequence, java.lang.CharSequence>> newValueMaps, List<Object> insertObjects, 
			Map<java.lang.CharSequence,java.lang.CharSequence> params) {		
		this.gPUdb = gPUdb;
		this.file = file;
		this.log = Logger.getLogger(BulkUpdateRequest.class);
		...
		*/
		...
		// build avro object
		bulk_update_request request = new bulk_update_request(ns.get_setid().get_id(), global_expression, expressions, newValueMaps,
		...
		if( gPUdb.isSnappyCompress() ) {
			try {
				...
				log.debug(" Compressed from " + bulk_update_data_bytes.length + " to :" + dataToSend.length);
			} catch (IOException e) {
				log.error(e);
	...
	private void createAuditMsg(bulk_update_request request) {
